...
export default function TurmasPage() {
    ...
    const fetchTurmas = useCallback(async (page: number = 1) => {
        try {
            setLoading(true)
            const response = await academicService.classes.getAll(page)
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-between bg-card/60 backdrop-blur-sm p-4 rounded-3xl border border-muted/30 shadow-sm mt-8">
                        <p className="text-sm text-muted-foreground font-medium">
                            Mostrando <span className="text-foreground font-bold">{turmas.length}</span> de <span className="text-foreground font-bold">{totalItems}</span> turmas
                        </p>
                        <div className="flex items-center gap-2">
                            <Button
                                variant="outline"
